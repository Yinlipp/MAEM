...
class MvposeAssociator:
    ...
    def process_data(self, mview_person_id, mview_img_arr, mview_bbox2d,
        ...
        for view, person_id in enumerate(mview_person_id):
            ...
            # 确保bbox2d数组是2D格式且为5列（包含置信度）
            bbox_array = v_bbox2d.numpy()
            original_shape = bbox_array.shape
            
            # 处理各种情况统一为 (N, 5) 格式
            if bbox_array.size == 0:
                # 完全空的数组
                bbox_array = np.empty((0, 5))
            elif bbox_array.ndim == 0:
                # 标量，转为空数组
                bbox_array = np.empty((0, 5))
            elif bbox_array.ndim == 1:
                if bbox_array.size == 4:
                    # 1D数组4个元素，添加置信度
                    bbox_array = np.append(bbox_array, 1.0).reshape(1, 5)
                elif bbox_array.size == 5:
                    # 1D数组5个元素，直接reshape
                    bbox_array = bbox_array.reshape(1, 5)
                else:
                    # 其他情况转为空数组
                    bbox_array = np.empty((0, 5))
                ...
                if bbox_array.shape[1] == 4:
                    # 2D数组4列，添加置信度列
                    conf_col = np.ones((bbox_array.shape[0], 1))
                    bbox_array = np.hstack([bbox_array, conf_col])
                elif bbox_array.shape[1] == 5:
                    # 2D数组5列，保持不变
                    pass
                else:
                    # 其他列数转为空数组
            ...
            # 确保最终结果是 (N, 5) 格式
            assert bbox_array.ndim == 2 and bbox_array.shape[1] == 5, \
                f"View {view}: bbox_array shape {bbox_array.shape} after processing, original: {original_shape}"
            
            ret_bbox2d.append(bbox_array)
