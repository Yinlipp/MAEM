...
def load_clip_from_mview_src(
    ...
    if img_arr is not None:
        ...
        ret_img_list = []
        # 动态获取第一个图像的分辨率作为参考
        first_img = cv2.imread(img_paths[0][start_idx])
        if first_img is None:
            raise FileNotFoundError(f"Cannot load first image: {img_paths[0][start_idx]}")
        
        target_height, target_width = first_img.shape[:2]
        ...
        for view_idx in range(len(img_paths)):
            ...
            for frame_idx in range(start_idx, end_idx):
                ...
                if img is not None:
                    # 保持宽高比的resize，使用INTER_AREA进行高质量缩放
                    ...
                    # 如果图像不存在，创建黑色图像
                    view_img_list.append(np.zeros((target_height, target_width, 3), dtype=np.uint8))
