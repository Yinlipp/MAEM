...
class TopDownAssociationEvaluation(BaseEvaluation):
    ...
    def run(self, overwrite: bool = False):
        ...
        pred_keypoints3d_raw = Keypoints(
            ...
            logger=self.logger)
        # Get actual GT convention from dataset if gt_kps3d_convention is None
        gt_convention = self.dataset.gt_kps3d_convention
        if gt_convention is None and len(self.dataset.gt3d) > 0:
            gt_convention = self.dataset.gt3d[0].get_convention()
        gt_keypoints3d_raw = Keypoints(
            dtype='numpy',
            kps=gt_kps3d,
            mask=gt_kps3d[..., -1] > 0,
            convention=gt_convention,
            logger=self.logger)
