...
class MPJPEMetric(BaseMetric):
    ...
    def __call__(self, pred_keypoints3d: Keypoints, gt_keypoints3d: Keypoints,
        ...
        mpjpe_mean, mpjpe_std = np.mean(masked_mpjpe_value), np.std(
            masked_mpjpe_value)
        mpjpe_median = np.median(masked_mpjpe_value)

        # AP@threshold: per-person mean MPJPE < threshold 的比例
        # mpjpe_value_gt2pred shape: (n_frame, n_person, n_kps)
        gt_mask_person = gt_mask.sum(axis=-1) > 0  # (n_frame, n_person)
        per_person_mpjpe = np.where(
            gt_mask > 0, mpjpe_value_gt2pred, np.nan
        ).mean(axis=-1)  # (n_frame, n_person), nan for invalid persons
        valid_person_mpjpe = per_person_mpjpe[gt_mask_person]  # 1D array

        ap_results = {}
        for thr in [75, 100, 125, 150]:
            ap_results[f'ap{thr}mm'] = float(
                (valid_person_mpjpe < thr).mean()) if len(valid_person_mpjpe) > 0 else 0.0

        return dict(
            mpjpe_mean=mpjpe_mean,
            mpjpe_median=mpjpe_median,
            mpjpe_std=mpjpe_std,
            **ap_results,
